  ...
  <div v-if="show">
    ...
    <div v-if="previewImage" class="preview-pane-wrapper">
      <div class="preview-container" :style="previewContainerStyle">
        ...
        <!-- Watermark Overlay -->
        <div class="watermark-overlay" v-if="watermarkData.text" :style="[watermarkStyles, watermarkPositionStyle]">
          {{ watermarkData.text }}
...
export default {
  ...
  computed: {
    ...
    watermarkPositionStyle() {
      ...
      temp.style.fontSize = `${this.watermarkData.size * this.previewScale}px`;
      temp.style.fontFamily = this.watermarkData.fontFamily;
      temp.style.fontWeight = this.watermarkData.bold ? "bold" : "normal";
      temp.style.fontStyle = this.watermarkData.italic ? "italic" : "normal";
      temp.style.textAlign = this.watermarkData.alignment || "center";
      temp.textContent = this.watermarkData.text || "WATERMARK";
      ...
      // Calculate position based on dimensions - matching App.vue logic
      const pageWidth = this.pageWidth * this.previewScale;
      const pageHeight = this.pageHeight * this.previewScale;

      const positions = {
        "top-left": { x: 0, y: 0 },
        "top-center": { x: (pageWidth - width) / 2, y: 0 },
        "top-right": { x: pageWidth - width, y: 0 },
        "middle-left": { x: 0, y: (pageHeight - height) / 2 },
        center: { x: (pageWidth - width) / 2, y: (pageHeight - height) / 2 },
        "middle-right": { x: pageWidth - width, y: (pageHeight - height) / 2 },
        "bottom-left": { x: 0, y: pageHeight - height },
        "bottom-center": { x: (pageWidth - width) / 2, y: pageHeight - height },
        "bottom-right": { x: pageWidth - width, y: pageHeight - height },
      };
      const pos = positions[this.watermarkData.position] || positions["center"];
...
.preview-container {
  @apply relative m-4 overflow-hidden self-center;
}
