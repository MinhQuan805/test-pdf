      ...
      pdfEditor.pdfPages.forEach((page) => {
        ...
        const addDrawingListeners = (element) => {
          ...
          element.addEventListener("mousemove", (event) => {
            if ((!isDrawing.value && !isAreaSelecting.value) || (selectedTool.value === "select" && !isAreaSelecting.value)) return;
          ...
          element.addEventListener("mouseup", (event) => {
            ...
            if (isAreaSelecting.value) {
              ...
              // Calculate selection rect in client coordinates
              const rect = container.getBoundingClientRect();
              const zoomFactor = zoomLevel.value;

              const startX = drawingStart.value.x * zoomFactor + rect.left;
              const startY = drawingStart.value.y * zoomFactor + rect.top;
              const endX = drawingCurrent.value.x * zoomFactor + rect.left;
              const endY = drawingCurrent.value.y * zoomFactor + rect.top;
              ...
              if (textLayer) {
                ...
                const selectedSpans = spans.filter(span => {
                  ...
                  return !(r.left > selectionRect.right ||
                    r.right < selectionRect.left ||
...
.textLayer {
  ...
  .highlight {
    --highlight-bg-color: rgba(0, 0, 255, 0.25);
    --highlight-selected-bg-color: rgba(0, 100, 200, 0.3);
